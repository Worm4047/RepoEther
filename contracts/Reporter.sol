...
contract Reporter {
    ...
    function getAllComplaints1() public constant returns (uint[], bytes32[], bytes32[],bytes32[],bytes32[],bytes32[]){
    ...
    function getAllComplaints2() public constant returns (bytes32[],bytes32[], uint[], uint[], bytes32[], uint[]){
	...
	function addStake(uint complaint_id, bytes32 user_address, uint stake, uint opinion) public payable {
		Stake memory newstake;
		newstake.user_address = user_address;
	...
	function distributeFunds(uint complaint_id, uint result) public payable {
		...
		for(i=0;i<stakes_on_complaint[complaint_id].length;i++){
			if(stakes_on_complaint[complaint_id][i].opinion != result)
				continue;
			uint amount = stakes_on_complaint[complaint_id][i].stake + extra_return;
			address to_address = address(stakes_on_complaint[complaint_id][i].user_address);
